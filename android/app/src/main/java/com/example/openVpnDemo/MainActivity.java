...
public class MainActivity extends FlutterActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        LocalBroadcastManager.getInstance(this).registerReceiver(new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (vpnStatusSink != null && attached) {
                    try {
                        ...
                        try {
                            ...
                            Log.w(TAG, "Flutter detached before sending VPN status", e);
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
            ...
            switch (call.method) {
                case "stop":
                    try {
                        ...
                        result.error("stop_exception", e.getMessage(), null);
                    }
                    break;
                case "start":
    ...
    private void setStage(String stage) {
        if (vpnStageSink != null && attached) {
            try {
                vpnStageSink.success(stage.toLowerCase());
            } catch (Exception e) {
                Log.w(TAG, "Flutter detached before sending VPN stage", e);
            }
        }
    }
}